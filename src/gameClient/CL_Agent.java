...
public class CL_Agent {
		...
		public CL_Agent(directed_weighted_graph g, int start_node) {
			_gg = g;
			setMoney(0);
			this.CurrNode = _gg.getNode(start_node);
		...
		private void setMoney(double v) {value = v;}

		public boolean setNextNode(int dest) {
			boolean ans = false;
			int src = this.CurrNode.getKey();
			this.CurrEdge = _gg.getEdge(src, dest);
			if(CurrEdge!=null) {
				ans=true;
			}
			else {CurrEdge = null;}
		...
		public void set_SDT(long ddtt) {
			...
			if(this.CurrEdge!=null) {
				...
				if(this.get_curr_fruit().get_edge()==this.get_curr_edge()) {
					 dist = currfood.getLocation().distance(this.pos);
				}
				double norm = dist/de;
